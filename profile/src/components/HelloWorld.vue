...
<template>
 ...
 <transition name="slide">
  <div v-if="isSidebarOpen" class="fixed inset-0 z-[60] flex">
    <div @click="isSidebarOpen = false" class="absolute inset-0 bg-black/95 backdrop-blur-md"></div>
    
    <aside class="relative w-80 bg-gray-950 h-full border-r border-gray-900 shadow-2xl flex flex-col">
      <div class="flex justify-between items-center px-8 py-7 border-b border-gray-900 bg-black/20">
        <h2 class="text-white font-black uppercase italic text-sm tracking-tighter">
          Marty Carps<span class="text-red-600">.</span>
        </h2>
        <span @click="isSidebarOpen = false" class="material-symbols-outlined text-gray-500 cursor-pointer hover:text-white transition-colors">close</span>
      </div>

      <nav class="flex flex-col px-4 py-8 gap-1 flex-grow">
        <a v-for="item in texts[currentLang].nav" :key="item.href" :href="item.href" @click="isSidebarOpen = false"
          class="text-gray-400 font-bold uppercase tracking-[0.2em] text-[10px] py-4 px-6 rounded-sm hover:bg-red-600/10 hover:text-white transition-all border-l-2 border-transparent hover:border-red-600">
          {{ item.label }}
        </a>
      </nav>

      <div class="p-8 border-t border-gray-900 bg-black/40 space-y-8">
        <div class="flex flex-col gap-4">
          <p class="text-[9px] font-black text-gray-600 uppercase tracking-[0.3em] italic">Select Language</p>
          <div class="flex gap-2">
            <button @click="changeLang('ES')" 
              :class="currentLang === 'ES' ? 'bg-red-600 text-white' : 'bg-gray-900 text-gray-500 border-gray-800'"
              class="flex-1 py-3 text-[10px] font-black rounded-sm border transition-all">ESPAÑOL</button>
            <button @click="changeLang('EN')" 
              :class="currentLang === 'EN' ? 'bg-red-600 text-white' : 'bg-gray-900 text-gray-500 border-gray-800'"
              class="flex-1 py-3 text-[10px] font-black rounded-sm border transition-all">ENGLISH</button>
          </div>
        </div>

        <div class="flex items-center justify-between pt-4">
          <div class="flex gap-6 items-center">
            <a href="https://github.com/martincarmna" target="_blank" class="text-gray-400 hover:text-white transition-colors">
              <svg class="w-6 h-6 fill-current" viewBox="0 0 24 24"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/></svg>
            </a>
            <span class="material-symbols-outlined text-gray-400 hover:text-white cursor-pointer text-2xl">search</span>
          </div>
          <p class="text-[8px] font-bold text-gray-700 uppercase tracking-widest">v2.0</p>
        </div>
      </div>
    </aside>
  </div>
